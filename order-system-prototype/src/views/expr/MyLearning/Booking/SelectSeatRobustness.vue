    ...
    <tr v-for="row in rows" :key="row">
      <td v-for="id in row" align="center" :key="id" @click="link(id)">
        <div class="seat" v-bind:id="id">
          <img
            width="50"
            src="../../../../assets/sofa-removebg-preview-small.png"
          />
          <br />
          {{ id }}
        </div>
...
export default {
  ...
  methods: {
    ...
    finish() {
      ...
      // });
      if (this.select == "") {
        ElMessage.error("Oops, this is a error happening.");
      } else {
        ElMessageBox.alert(
          "您已成功预定一个座位,id:" + this.select,
          "预定成功",
          {
          ...
          // router.push("/leave");
          axios.post("/api/stop_record").then((response) => {
    ...
    link(id) {
      for (let row in this.rows) {
        for (let seat in this.rows[row]) {
          if (
            document.getElementById(this.rows[row][seat]).style
              .backgroundColor != "" &&
            document.getElementById(this.rows[row][seat]).id != id
          ) {

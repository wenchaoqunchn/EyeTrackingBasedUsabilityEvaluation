        ...
        if data["fixation"]:
            ...
            if len(tmp_data) > 0:
                ...
                tmp = {
                    ...
                    "duration": tmp_data[-1]["timestamp_unix"] - tmp_data[0]["timestamp_unix"],
                }
                if tmp["duration"] != 0:
...
def mark_fixation_and_saccade(eye_track_data):
    ...
    for i in range(1, len(eye_track_data)):
        ...
        if eye_track_data[i]["validity"] == "invalid" or eye_track_data[i - 1]["validity"] == "invalid":
            continue
        ...
        time = eye_track_data[i]['timestamp_unix'] - eye_track_data[i - 1]['timestamp_unix']
        if time == 0:
            time = 0.00001
        speed = distance / time
        # print(speed)
...
def draw_eye_track_on_image(image, eye_track_data):
    ...
    for i, point in enumerate(points):
        ...
        # draw.ellipse((x - 5, y - 5, x + 5, y + 5), fill=(255 - i * color_step, 0, 0))
        if fixation:
            draw.ellipse((x - 20, y - 20, x + 20, y + 20), fill=(255 - i * color_step, 0, 0))
        else:
            draw.ellipse((x - 5, y - 5, x + 5, y + 5), fill=(255 - i * color_step, 0, 0))
    return image
...
def draw_eye_track_on_image_with_merged_fixation(image, eye_track_data):
    ...
    for i, point in enumerate(points):
        ...
        if point["fixation"]:
            ...
            draw.rectangle(((point["x"] - 10, point["y"] - 10), (point["x"] + 10, point["y"] + 10)), fill=None,
                           outline=(0, 0, 255), width=5)
        else:
            draw.ellipse((point["x"] - 5, point["y"] - 5, point["x"] + 5, point["y"] + 5),
...
def count_and_duration_fixation_overlap_aoi(AOIs_size, eye_track_data):
    ...
    for i, data in enumerate(eye_track_data):
        ...
        for aoi in AOIs_size:
            ...
            if is_fixation_circle_overlap_AOI((aoi["x1"], aoi["x2"], aoi["y1"], aoi["y2"]), (x, y, r), 0):
                data["aoi"] = aoi["name"]
                fixation_on_aoi += 1
    ...
    fixation_rate = 0 if total_fixation_count == 0 else fixation_on_aoi / total_fixation_count
    ...
    for i, data in enumerate(eye_track_data):
        ...
        if data["aoi"] != "":
            ...
            for aoi in not_relevant_aois:
                if is_fixation_circle_overlap_AOI((aoi["x1"], aoi["x2"], aoi["y1"], aoi["y2"]),
                                                  (data["x"], data["y"], data["radius"]), 0):
                    data["aoi"] = aoi["name"]
...
def write_to_excel(data, sheet_name, NFRs, exp, result_path):
    ...
    pd.concat([df, df2]).to_csv(result_path+"统计_" + sheet_name + ".csv", index=False, encoding="utf-8")

    for NFR in NFRs:
        if exp:
            df_tmp = pd.DataFrame({'Metrics': ['fixation_count', 'duration', 'fixation_rate', 'first_fixation_on_aoi', 'scan_path_accuracy', 'edit_distance', 'attention_switch', 'convex_hull'],
                                    'Exp': [data["fixation_count"], data["duration"], data["fixation_rate"], data["first_fixation_on_aoi"], data["scan_path_accuracy"], data["edit_distance"], data["attention_switch"], data["convex_hull"]],
                                   'Standard': ["", "", "", "", "", "", "", ""]})
        else:
            df_tmp = pd.DataFrame({'Metrics': ['fixation_count', 'duration', 'fixation_rate', 'first_fixation_on_aoi', 'scan_path_accuracy', 'edit_distance', 'attention_switch', 'convex_hull'],
                                    'Exp': ["", "", "", "", "", "", "", ""],
                                    'Standard': [data["fixation_count"], data["duration"], data["fixation_rate"], data["first_fixation_on_aoi"], data["scan_path_accuracy"], data["edit_distance"], data["attention_switch"], data["convex_hull"]]})

        try:
            df = pd.read_excel(result_path+"统计" + NFR + ".xlsx")
            pd.concat([df, df_tmp]).to_excel(result_path+"统计"+NFR+".xlsx", index=False)
        except:
...
def paint(root_path, time_stamp, result_path):
    ...
    for i in range(len(components)):
        ...
        fixation_count, duration, fixation_rate, first_fixation_on_aoi = count_and_duration_fixation_overlap_aoi(
            AOIs_size, time_window_eye_track)
        ...
        result = {"unix_timestamp": str(components[i]["unix_timestamp"]),
                  "path": path,
    ...
    if bad_designs["Accessibility"]:
        if "/expr/MyHome/News" in results:
            write_to_excel(results["/expr/MyHome/News"], "Accessibility_true", ["Accessibility"], True, result_path)
    else:
        if "/expr/MyHome/News" in results:
            write_to_excel(results["/expr/MyHome/News"], "Accessibility_false", ["Accessibility"], False, result_path)

    if bad_designs["Ease_of_use"]:
        if "/expr/MyLearning/Booking/SelectDateEaseOfUse" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectDateEaseOfUse"], "Ease_of_use_true", ["Ease_of_use"], True, result_path)
    elif bad_designs["Consistency"]:
        if "/expr/MyLearning/Booking/SelectDateConsistency" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectDateConsistency"], "Consistency_true", ["Consistency"], True, result_path)
    else:
        if "/expr/MyLearning/Booking/SelectDate" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectDate"], "Ease_of_use_&_Consistency_false", ["Ease_of_use", "Consistency"], False, result_path)

    if bad_designs["Accuracy"]:
        if "/expr/MyLearning/Booking/SelectSpace" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectSpace"], "Accuracy_true", ["Accuracy"], True, result_path)
    else:
        if "/expr/MyLearning/Booking/SelectSpace" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectSpace"], "Accuracy_false", ["Accuracy"], False, result_path)

    if bad_designs["Device_efficiency"]:
        if "/expr/MyLearning/Booking/SelectSeatDeviceEfficiency" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectSeatDeviceEfficiency"], "Device_efficiency_true", ["Device_efficiency"], True, result_path)
    elif bad_designs["Robustness"]:
        if "/expr/MyLearning/Booking/SelectSeatRobustness" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectSeatRobustness"], "Robustness_true", ["Robustness"], True, result_path)
    else:
        if "/expr/MyLearning/Booking/SelectSeat" in results:
            write_to_excel(results["/expr/MyLearning/Booking/SelectSeat"], "Device_efficiency_&_Robustness_false", ["Device_efficiency", "Robustness"], False, result_path)
